...
class Database(list):
    ...
    def generate_html(self):
            ...
            templates.ways.substitute(
                {
                    ...
                    "content": "\n".join(
                        templates.way.substitute(
                            dict((k, escape(str(v))) for k, v in way.items())
                            | {"char_count": len(way["command"])}
                        )
                        for way in sorted(
                            ways, key=lambda x: (x["technology"].name, x["command"])
                        )
                    ),
                    "id": ways.id,
        ...
        site_content = templates.site.substitute(
            {"content": content, "toc": table_of_contents}
        )
...
ways.add(
    technology=Ruby,
    command="""ruby -e 'ARGV.each{|f| IO.readlines(f).each{|l| puts "#{f}:#{l}"}' *.log""",
)
